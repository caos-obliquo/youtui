...
impl DominantKeyRouter<AppAction> for YoutuiWindow {
    ...
    fn get_dominant_keybinds<'a>(
        ...
        match self.context {
            ...
            WindowContext::Notes => Either::Right(Either::Right(
                [&config.keybinds.help, &config.keybinds.list].into_iter(),
            )),
        }
    }
        ...
        match self.context {
            ...
            WindowContext::PlaylistEditor => {
                if let Some(ref mut ed) = self.playlist_editor_popup {
                    ...
                    let new_cursor = (ed.cursor as isize)
...
impl YoutuiWindow {
    ...
    pub fn get_help_list_items(&self) -> Vec<DisplayableKeyAction<'_>> {
        let mut items: Vec<DisplayableKeyAction<'_>> = match self.context {
            ...
            WindowContext::PlaylistSavePopup => get_visible_keybinds_as_readable_iter(
                std::iter::once(&self.config.keybinds.playlist_save_popup),
            )
            .collect(),
            WindowContext::PlaylistUpdatePopup => get_visible_keybinds_as_readable_iter(
                std::iter::once(&self.config.keybinds.playlist_save_popup),
            )
            .collect(),
    ...
    pub async fn handle_crossterm_event(
            ...
            && let Event::Key(k) = event
        {
            ...
            let effect =
                effect.map_frontend(|this: &mut Self| this.config_editor_popup.as_mut().unwrap());
            ...
            && let Event::Key(k) = event
        {
            ...
            let (effect, callback) = popup.handle_key(k);
            let effect = effect.map_frontend(|this: &mut Self| this.notes_popup.as_mut().unwrap());
            return YoutuiEffect { effect, callback };
        }
        ...
        if self.command_mode {
                ...
                && k.kind == crossterm::event::KeyEventKind::Press
            {
                ...
                let submitted = self.command_editor.handle_key(
                    k.code,
                ...
                if submitted {
                    ...
                    if !cmd.is_empty() {
                        ...
                        if cmd == "reload" || cmd == "reload!" {
                            return YoutuiEffect {
                                effect: AsyncTask::new_no_op(),
                                callback: Some(AppCallback::ReloadConfig),
                            };
                        }
                        if cmd == "notes" {
                            ...
                            self.notes_popup =
                                Some(crate::app::ui::playlist::notes_popup::NotesPopup::new(
                                    notes_path, content,
                                ));
                            self.prev_context = self.context;
            ...
            && let Event::Key(k) = event
        {
            ...
            let (effect, callback) = popup.handle_key(k);
            let effect = effect.map_frontend(|this: &mut Self| this.lyrics_popup.as_mut().unwrap());
            return YoutuiEffect { effect, callback };
        }
            ...
            && let Event::Key(k) = event
        {
            ...
            let (effect, callback) = popup.handle_key(k);
            let effect =
                effect.map_frontend(|this: &mut Self| this.album_art_popup.as_mut().unwrap());
            return YoutuiEffect { effect, callback };
        }
        if self.song_info_popup.is_some()
            && let Event::Key(k) = event
        {
            ...
            let (effect, callback) = popup.handle_key(k);
            let effect =
                effect.map_frontend(|this: &mut Self| this.song_info_popup.as_mut().unwrap());
            return YoutuiEffect { effect, callback };
        }
        if self.playlist_save_popup.is_some()
            && let Event::Key(k) = event
        {
            ...
            let (effect, callback) = popup.handle_key(k);
            let effect =
                effect.map_frontend(|this: &mut Self| this.playlist_save_popup.as_mut().unwrap());
            return YoutuiEffect { effect, callback };
        }
        if self.playlist_update_popup.is_some()
            && let Event::Key(k) = event
        {
            ...
            let (effect, callback) = popup.handle_key(k);
            let effect =
                effect.map_frontend(|this: &mut Self| this.playlist_update_popup.as_mut().unwrap());
            return YoutuiEffect { effect, callback };
        }
            ...
            && let Event::Key(k) = event
        {
            ...
            let (effect, callback) = popup.handle_key(k);
            let effect =
                effect.map_frontend(|this: &mut Self| this.playlist_rename_popup.as_mut().unwrap());
            return YoutuiEffect { effect, callback };
        }
            ...
            && let Event::Key(k) = event
        {
            ...
            let (effect, callback) = popup.handle_key(k);
            let effect =
                effect.map_frontend(|this: &mut Self| this.playlist_edit_popup.as_mut().unwrap());
            return YoutuiEffect { effect, callback };
        }
            ...
            && let Event::Key(k) = event
        {
            ...
            let (effect, callback) = popup.handle_key(k);
            let effect = effect
                .map_frontend(|this: &mut Self| this.playlist_details_popup.as_mut().unwrap());
            return YoutuiEffect { effect, callback };
        }
            ...
            && let Event::Key(k) = event
        {
            ...
            let (effect, callback) = popup.handle_key(k);
            let effect =
                effect.map_frontend(|this: &mut Self| this.playlist_editor_popup.as_mut().unwrap());
            return YoutuiEffect { effect, callback };
        }
    ...
    pub fn handle_key_event(
        ...
        let count_prefix_active = matches!(
            self.context,
            WindowContext::Playlist | WindowContext::Browser
        );
    ...
    pub fn handle_text_entry_action(&mut self, action: TextEntryAction) -> ComponentEffect<Self> {
        ...
        match self.context {
            ...
            WindowContext::Notes => AsyncTask::new_no_op(),
        }
    }
    ...
    pub fn handle_insert_next(&mut self, song_list: Vec<ListSong>) -> ComponentEffect<Self> {
        let (_, effect) = self.playlist.insert_next_song_list(song_list);
        effect.map_frontend(|this: &mut Self| &mut this.playlist)
    }
    pub fn handle_queue_song(&mut self, song_list: Vec<ListSong>) -> ComponentEffect<Self> {
        let (_, effect) = self.playlist.push_song_list(song_list);
        effect.map_frontend(|this: &mut Self| &mut this.playlist)
    }
    ...
    pub fn open_playlist_save_popup(
        &mut self,
        video_ids: Vec<ytmapi_rs::common::VideoID<'static>>,
    ) {
    ...
    pub fn open_playlist_update_popup(
        ...
        use crate::app::ui::playlist::effect_handlers_playlist::{
            HandleGetAllLibraryPlaylistsError, HandleGetAllLibraryPlaylistsOk,
        };
    ...
    pub fn open_playlist_merge_popup(
        &mut self,
        source_playlist_id: ytmapi_rs::common::PlaylistID<'static>,
    ) -> ComponentEffect<Self> {
        use crate::app::server::GetAllLibraryPlaylists;
        use crate::app::ui::playlist::effect_handlers_playlist::{
            HandleGetAllLibraryPlaylistsError, HandleGetAllLibraryPlaylistsOk,
        };
        let sid = source_playlist_id.clone();
        self.playlist_update_popup = Some(PlaylistUpdatePopup::new_merge(source_playlist_id));
        self.prev_context = self.context;
        self.context = WindowContext::PlaylistUpdatePopup;
        AsyncTask::new_future_try(
            GetAllLibraryPlaylists,
        ...
        .map_frontend(move |this: &mut Self| {
            if this.playlist_update_popup.is_none() {
                this.playlist_update_popup = Some(PlaylistUpdatePopup::new_merge(sid));
            }
            this.playlist_update_popup.as_mut().expect("just set")
        })
    }
    pub fn open_lyrics_popup(&mut self, artist: String, title: String) -> ComponentEffect<Self> {
        ...
        use crate::app::ui::playlist::effect_handlers_playlist::{
            HandleGetAnnotationsErr, HandleGetAnnotationsOk, HandleGetLyricsErr, HandleGetLyricsOk,
        };
        let genius_token = self.config.scrobbling.genius_token.clone();
        tracing::info!(
            "open_lyrics_popup: artist={}, title={}, token_len={}",
            artist,
            title,
            genius_token.len()
        );
        ...
        self.lyrics_viewing_idx = self.playlist.list.get_list_iter().position(|s| {
            s.title == title
                && s.artists
                    .iter()
                    .any(|a| artist.contains(a.name.as_str()) || a.name.contains(&artist))
        });

        // Inflight dedup: skip if already fetching
        if self.lyrics_inflight.contains(&cache_key) {
            tracing::info!(
                "Lyrics inflight dedup: skip duplicate request for {}",
                cache_key
            );
            return AsyncTask::new_no_op();
        }
        ...
        if let Some(popup) = &self.lyrics_popup {
            ...
            {
                tracing::info!(
                    "Lyrics negative cache hit for {} (age={}s)",
                    cache_key,
        ...
        // Preserve caches from previous popup so LRU works across songs
        if let Some(old) = &self.lyrics_popup {
            for (k, v) in old.lyrics_cache.iter() {
                popup.lyrics_cache.put(k.clone(), v.clone());
            }
            for (k, v) in old.error_cache.iter() {
                popup.error_cache.put(k.clone(), *v);
            }
        }
        ...
        .map_frontend(move |this: &mut Self| {
            if this.lyrics_generation != gen_id {
    ...
    pub fn open_song_info_popup(
        &mut self,
        song: crate::app::structures::ListSong,
    ) -> ComponentEffect<Self> {
    ...
    pub fn close_popup(&mut self) {
        ...
        if self.context == WindowContext::Notes && self.notes_popup.is_some() {
            tracing::info!(
                "close_popup: notes stacked -> restore prev_context={:?}, lyrics_popup={}",
                self.prev_context,
                self.lyrics_popup.is_some()
            );
    ...
    pub fn open_playlist_rename_popup(
        &mut self,
    ...
    pub fn open_playlist_details_popup(
        &mut self,
        playlist_id: PlaylistID<'static>,
        title: String,
    ) -> ComponentEffect<Self> {
        ...
        use crate::app::ui::playlist::effect_handlers_playlist::{
            HandleFetchPlaylistDetailsError, HandleFetchPlaylistDetailsOk,
        };
    ...
    fn get_cur_displayable_mode(
        ...
        let displayable_commands: Vec<DisplayableKeyAction<'_>> = keys
            .iter()
            .filter(|(_, kt)| {
        ...
        let description: std::borrow::Cow<'_, str> = if is_library {
            format!(
                "{} [{}]",
                name,
                self.browser.library_browser.category.label()
            )
            .into()
        } else {
            name.into()
...
#[cfg(test)]
mod tests {
    use super::{YoutuiWindow, extract_playlist_id, extract_video_id};
    use crate::config::Config;
    ...
    fn extract_playlist_id_album_url() {
        ...
        assert_eq!(
            ...
            Some("OLAK5uy_n7A1VkPqP_8RkGeVh5H".to_string())
        );
    }
