...
use crate::app::ui::playlist::effect_handlers::{
    ...
    HandleSongDownloadProgressUpdate, HandleStopped, HandleVolumeUpdate, HandleSaveQueueOk, HandleSaveQueueError,
};
...
impl Action for PlaylistAction {
    ...
    fn describe(&self) -> std::borrow::Cow<'_, str> {
        match self {
            ...
            PlaylistAction::CreatePlaylist => "Create New Playlist",
            PlaylistAction::SaveQueueToPlaylist => "Save Queue to Playlist",
...
let video_ids: Vec<VideoID<'static>> = self.list.get_list_iter()
    ...
    AsyncTask::new_future_try(
        CreatePlaylistWithVideos {
            title: playlist_name,
            description: Some("Created from Youtui queue".to_string()),
            video_ids,
        },
        HandleSaveQueueOk,
