...
use crate::app::structures::{
    AlbumArtState, DownloadStatus, ListSong, ListSongArtist, ListSongDisplayableField, ListSongID,
    ListStatus, MaybeRc, PlayState,
};
use crate::app::ui::playlist::{
    DownloadTask, HandleGetSongThumbnailError, HandleGetSongThumbnailOk, HandlePlayUpdateError,
    HandlePlayUpdateOk, Playlist, QueueState,
};
...
use ytmapi_rs::common::{AlbumID, LikeStatus, Thumbnail, VideoID, YoutubeID};
...
async fn autoplay_song_id_sets_up_scrobble_state() {
    ...
    assert!(
        p.scrobble_state.is_some(),
        "autoplay must create scrobble_state"
    );
}
...
fn insert_album_tracks_sets_correct_metadata() {
    ...
    let orig_id = p
        .list
        .push_song_list(vec![make_album_original("vx1", Some("2021"))]);
    ...
    // Track 3: last track, offset 203+148 = 351, uses track duration
    // (parent_duration is None)
...
fn insert_album_tracks_propagates_parent_metadata() {
    ...
    for i in 0..3 {
        ...
        assert_eq!(
            track.genres,
            vec!["Rock", "Metal"],
            "Track {}: genres should propagate",
            i + 1
        );
        assert_eq!(
            track.styles,
            vec!["Heavy Metal"],
            "Track {}: styles should propagate",
            i + 1
        );
        assert_eq!(
            track.like_status,
            LikeStatus::Liked,
            "Track {}: like_status should propagate",
            i + 1
        );
...
fn album_download_shares_arc_with_tracks() {
    ...
    let orig_id = p
        .list
        .push_song_list(vec![make_album_original("vx1", None)]);

    // Real flow: MetadataEffect::Validated sets album_tracks BEFORE
    // insert_album_tracks
    let tracks = vec![
        AlbumTrack {
            title: "T1".into(),
    ...
    p.list.get_list_iter_mut().nth(0).unwrap().download_status =
        DownloadStatus::Downloaded(shared.clone());
...
fn cancel_all_downloads_multiple_tasks() {
    ...
    p.active_downloads.lock().unwrap().push((
        ListSongID(1),
        DownloadTask {
