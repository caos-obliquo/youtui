...
impl ActionHandler<ConfigEditorAction> for ConfigEditorPopup {
    fn apply_action(&mut self, action: ConfigEditorAction) -> impl Into<YoutuiEffect<Self>> {
        match action {
            ConfigEditorAction::Close => (AsyncTask::new_no_op(), Some(AppCallback::ClosePopup)),
        }
    }
}
...
impl ConfigEditorPopup {
    ...
    pub fn handle_key(
        ...
        match event.code {
            ...
            _ => {
                // ZZ: save and quit
                if event.code == KeyCode::Char('Z') && self.editor.mode == ViMode::Normal {
                    // Wait for next key via key stack? No - handle key chords
                    // here. We can't do ZZ in a single
                    // handle_key call. We'll handle it via
                    // the key_stack mechanism.
                }
    ...
    pub fn draw(&mut self, frame: &mut Frame, area: Rect) {
        ...
        let block = Block::default()
            .title(format!(
                " Config Editor {mode} (Ctrl+s: Save, Esc: Cancel) "
            ))
            .borders(Borders::ALL)
