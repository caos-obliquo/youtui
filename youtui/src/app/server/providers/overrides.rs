...
impl OverrideEntry {
    fn has_data(&self) -> bool {
        self.album.is_some() || self.year.is_some() || self.genres.is_some() || self.styles.is_some()
    }
...
impl MetadataOverrides {
    ...
    pub fn set(
        ...
        let entry = OverrideEntry {
            ...
            genres: if meta.genres.is_empty() { None } else { Some(meta.genres.clone()) },
            styles: if meta.styles.is_empty() { None } else { Some(meta.styles.clone()) },
        };
        ...
        if has_genre_data {
            ...
            self.artist_genres.insert(artist.to_lowercase(), artist_entry);
        }
    }
}
