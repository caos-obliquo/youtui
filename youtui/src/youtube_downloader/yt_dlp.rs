    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            YtDlpDownloaderError::FormatNotAvailable { video_id } => {
                write!(
                    f,
                    "Error running yt-dlp - format not available for video {}",
                    video_id
                )
            }
            YtDlpDownloaderError::AuthenticationError { video_id, message } => {
                write!(
                    f,
                    "Error running yt-dlp - authentication failed for video {}: {}",
                    video_id, message
                )
            }
...
impl YoutubeMusicDownloader for YtDlpDownloader {
    ...
    > {
        ...
        async move {
            ...
            // Stdout pipe (-o -) skips post-processing, producing corrupted data on yt-dlp
            // 2026+. Use .m4a suffix so yt-dlp matches the format container.
            ...
            let format_string =
                "bestaudio[ext=m4a][abr>=256]/bestaudio[ext=m4a]/bestaudio/best".to_string();
            ...
            let status = timeout(Duration::from_secs(300), proc.wait())
                .await
                .map_err(|_| {
            ...
            } else if file_bytes.starts_with(b"\x1a\x45\xdf\xa3") {
                "WebM"
            } else if file_bytes.starts_with(b"RIFF") {
                "WAV"
            } else if file_bytes.starts_with(b"OggS") {
                "Ogg"
            } else {
                "unknown"
            };
            ...
            let is_valid = total_size_bytes > 100 && format_name != "unknown";
            ...
            if !is_valid {
                let info = if file_bytes.is_empty() {
                    ...
                    format!(
                        "invalid header: {:02x?}",
                        &file_bytes[..16.min(total_size_bytes)]
                    )
                };
...
mod tests {
    ...
    async fn test_downloading_a_song_with_ytdlp() {
        ...
        let YoutubeMusicDownload { song: stream, .. } = downloader
            .stream_song("lYBUbBu4W08", AudioQuality::default())
            .await
            .unwrap();
        stream
